...
public class RedUtils
{
    ...
    public boolean isRight(float money, int count)
    {
        ...
        else if (avg > MAXVALUE) { return false; }
        return true;
    }
    ...
    public static List<Float> getRed(float money, int count)
    {
        RedUtils red = new RedUtils();
        List<Float> org = red.spiltRedPackets(money, count);
        List<Float> res = new ArrayList<>();
        for (Float f : org)
        {
            BigDecimal b = new BigDecimal(f);
            res.add(b.setScale(2, BigDecimal.ROUND_HALF_UP).floatValue());
        }
        return res;
    }
}
